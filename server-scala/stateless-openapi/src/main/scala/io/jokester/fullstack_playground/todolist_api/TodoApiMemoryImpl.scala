...
class TodoApiMemoryImpl extends TodoApiService with LazyLogging with Lifters {

  private val _nextId = new AtomicInteger(0)
  private val _todos  = new AtomicReference[Map[Int, Todo]](Map.empty)
  private val clock   = Clock.systemUTC()

  override def list(): Failable[TodoList] = liftSuccess(TodoList(_todos.get.values.toSeq))
